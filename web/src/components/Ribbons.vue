...
export default {
	...
	methods: {
		themeChange() {
			localStorage.setItem("darkTheme", this.darkStyle);
			let myCanvas = document.getElementById("myCanvas");
			if (this.darkStyle) {
				myCanvas.style["backgroundColor"] = "#1f1f1f";
				this.submitBtnStyle =
	...
	created() {
		this.$nextTick(() => {
			let darkStyle = localStorage.getItem("darkTheme") == "true";
			const myDate = new Date();
			if (
				!darkStyle &&
				(myDate.getHours() >= 22 || myDate.getHours() <= 6) // 夜间22:00至第二天早上7:00，将自动切换至深色模式
			) {
				darkStyle = true;
				this.$message({
					dangerouslyUseHTMLString: true,
					message:
						"<i class='el-icon-moon'></i>夜深了，系统已自动为您切换至深色模式。请爱护好自己的眼睛哦！<i class='el-icon-view'></i>",
					type: "",
					duration: 2000,
					showClose: false,
					center: true,
			...
			this.$emit("update:darkStyle", darkStyle);
			if (this.darkStyle) {
				this.submitBtnStyle =
					"background-color: gold; border-color: gold; color: #1f1f1f";
			}
			...
			Factory.prototype = {
				...
				addRibbon: function () {
					...
					if (/^(top|min)$/i.test(this._options.verticalPosition)) {
						...
						/^(middle|center)$/i.test(
							this._options.verticalPosition
						)
					) {
