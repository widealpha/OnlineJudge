<template>
	<div class="list">
		<el-container>
			<link href="//at.alicdn.com/t/font_2807167_dodqs5cvnm7.css" rel="stylesheet">
			<el-main>
				<el-button v-if="isTeacher" type="primary" @click="showAddUserList=true">
					添加用户组
				</el-button>
				<el-table v-loading="loading" :data="userList" border stripe style="width: 100%">
					...
					<el-table-column fixed label="类型" prop="type" show-overflow-tooltip width="100">
					</el-table-column>
					<el-table-column fixed label="创建时间" prop="create_time" show-overflow-tooltip width="200">
					</el-table-column>
					<el-table-column v-if="!isTeacher" fixed label="是否绑定" min-width="100" prop="bind" show-overflow-tooltip>
						<template slot-scope="scope" style="text-align: center">
							<span v-if="scope.row.bind===0" style="cursor: pointer" @click="bindToUserList(scope.row.id)">
								<i class="iconfont icon-click" style="color: #179cff;font-size: 25px" />
								点击绑定
							</span>
					...
					<el-table-column v-if="isTeacher" label="操作" width="150">
						<template slot-scope="scope">
							...
							<el-button size="small" type="text" @click.native.prevent="deleteRow(scope.$index)">
								移除
							</el-button>
						</template>
					</el-table-column>
		...
		<el-dialog :visible.sync="comfirmDel" title="确认删除">
			...
			<div class="deleteBtn" @click="checkDel">
				确认删除
			</div>
		</el-dialog>
		<div v-show="showAddUserList" class="add">
			<el-dialog :visible.sync="showAddUserList" title="添加用户组">
				...
				<el-button style="margin-left: 10px" type="primary" @click="addUserList">
					添加用户组
				</el-button>
			</el-dialog>
		</div>
