<template>
	<div>
					...
					<div class="container" style="text-align: left">
						<div class="avatar" style="display: flex;   ">
							<el-tooltip class="item" effect="dark" placement="right-end">
								<div slot="content">为了保障您的使用体验，<br />请尽可能上传2M以下的图片</div>
								<el-upload class="avatar-uploader" action="" :show-file-list="false" :on-change="imgCropper" :auto-upload="false" ref="upload">
									<el-image style="width: 10vw; height: 10vw; float: left" :src="avatar"></el-image>
								</el-upload>
							</el-tooltip>
							...
							<div style="margin-left: 50px">
								...
								<div style="margin-top: 10px" v-if="!needBind">
									<el-button type="danger" @click="deleteBind">账号解绑</el-button>
								</div>
							</div>
						</div>
						<el-divider content-position="left"><b>基本信息</b></el-divider>
						<div class="baseInfo">
							<el-form label-position="left" :model="bindForm" ref="bindForm" :rules="bindRules" :hide-required-asterisk="true" :inline-message="true">
								...
								<el-form-item prop="name" label="姓名：">
									<el-input v-if="inputingMyInfo" style="width: 10vw;" placeholder="请输入姓名" v-model="bindForm.name"></el-input>
									<span class="infos">{{myInfo.name}}</span>
								</el-form-item>
								<el-form-item prop="stuNum" label="学号：">
									<el-input v-if="inputingMyInfo" style="width: 10vw;" placeholder="请输入学号" v-model="bindForm.stuNum"></el-input>
									<span class="infos">{{myInfo.stuNum}}</span>
								</el-form-item>
								<el-form-item v-if="needBind" prop="code" label="绑定码：">
									<el-input v-if="inputingMyInfo" style="width: 10vw;" placeholder="请输入绑定码" v-model="bindForm.code"></el-input>
								</el-form-item>
							</el-form>
							<el-button v-if="needBind&&!inputingMyInfo" @click="inputingMyInfo=true;">用户绑定</el-button>
							<el-button v-if="inputingMyInfo" @click="bind">提交绑定信息</el-button>
						</div>
					</div>
				</el-card>
			</el-main>
		...
		<div class="others">
			...
			<el-dialog title="修改密码" :visible.sync="isUpdatingPsw" @open="$refs.updatePswForm?$refs.updatePswForm.resetFields():''">
				<el-form :model="updatePswForm" ref="updatePswForm" :rules="updatePswRules">
					<el-form-item label="旧密码" prop="oldPsw">
						<el-input type="password" @keydown.enter.native="updatePsw" v-model="updatePswForm.oldPsw" placeholder="请输入原来的密码" prefix-icon="el-icon-key"></el-input>
					</el-form-item>
					<el-form-item label="新密码" prop="newPsw">
						<el-input type="password" @keydown.enter.native="updatePsw" v-model="updatePswForm.newPsw" placeholder="请输入新的密码" prefix-icon="el-icon-key"></el-input>
					</el-form-item>
				</el-form>
	...
	methods: {
		async getMyInfo() {
			...
			if (res.data.code == 0) {
				...
				this.$set(this.myInfo, "name", res.data.data.realName);
				this.needBind = res.data.data.stuNum == null;
			}
		},
		initInfo() {
			let jwt = require("jsonwebtoken");
			let token = jwt.decode(this.$store.state.token);
			if (token.ROLE.indexOf("ROLE_ADMIN") != -1) {
				this.$set(this.myInfo, "role", "管理员");
			} else if (token.ROLE.indexOf("ROLE_TEACHER") != -1) {
				this.$set(this.myInfo, "role", "教师");
			} else {
				this.$set(this.myInfo, "role", "学生");
			}
			this.avatar = require("@/assets/icon/student.svg");
			this.getMyInfo();
		},
		...
		setAvatar() {
			...
			this.$refs.cropper.getCropBlob((data) => {
				let file = new window.File([data], "test");
				console.log(file);
				let reader = new FileReader();
				reader.onload = async () => {
